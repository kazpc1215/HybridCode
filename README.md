...
i, j粒子間の加加速度計算。
$G m_{\ast}  \left[ \frac { (\vec{v}_j - \vec{v}_i) } { ({r_{ij}^2 + \varepsilon^2 )}^{3/2} } - 3 \frac { (\vec{v}_j - \vec{v}_i) \cdot (\vec{x}_j - \vec{x}_i) \right) r _ { i } } { \left| r _ { i } \right| ^ { 5 } } \right]$
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTMwMjQ1MTc5MSwtNDQ2Mjc2MTI1LC0xOT
