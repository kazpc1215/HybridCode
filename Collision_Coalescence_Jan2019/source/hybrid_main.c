...
#pragma omp parallel
  ...
  if(stat(endfile,&st)==0){  //読み込むファイルの存在確認.
    ...
    */
    ...
    ele[1].mass = IsolationMass(ele[1].axis,DELTA_HILL,SIGMA_SOLID,ALPHA_SOLID);
    for(i=2;i<=global_n_p;++i){
      ele[i].axis = Iteration_of_InitialAxis(ele[i-1].axis,ele[i-1].mass,DELTA_HILL,SIGMA_SOLID,ALPHA_SOLID);
      ele[i].mass = IsolationMass(ele[i].axis,DELTA_HILL,SIGMA_SOLID,ALPHA_SOLID);
    }


    double mass_p_tot=0.0;
    for(i=1;i<=global_n_p;++i){
      mass_p_tot += ele[i].mass;
      fprintf(fplog,"axis%02d = %e[AU],\tmass%02d = %e[M_Sun]\n",i,ele[i].axis,i,ele[i].mass);
    }
    fprintf(fplog,"mass_p_tot = %e[M_Sun] = %e[M_Earth]\n",mass_p_tot,mass_p_tot/3.0E-6);
  ...
  while(t_sys + t_tmp <= T_MAX){
	...
	fprintf(fpcollision,"#1:x_G\t2:y_G\t3:z_G\t4:vx_G\t5:vy_G\t6:vz_G\n");
	fprintf(fpcollision,"%.15e\t%.15e\t%.15e\t%.15e\t%.15e\t%.15e\n",
		x_0[0][1],
		x_0[0][2],
		x_0[0][3],
		v_0[0][1],
	...
	fprintf(fpcollision,"\n\n");
	fprintf(fpcollision,"#1:t[yr]\t2:i\t3:x\t4:y\t5:z\t6:r_0(3D)\t7:r_0(2D)\t8:v_x\t9:v_y\t10:v_z\t11:abs_v\t12:R_Hill\t13:Radius\t14:mass\n");
	for(i=1;i<=global_n;i++){
	  if(i==i_col || i==j_col){
...
#if COLLISION_FILE
	//衝突直後の位置速度をtempファイルへ書き出し.
	sprintf(collisionfile,"%sCollision_temp_%03d.dat",
		#ifdef SUBDIRECTORY
		dirname
#else
		STR(DIRECTORY)
	...
	fpcollision= fopen(collisionfile,"w");
	if(fpcollision==NULL){
	  fprintf(fplog,"collision tempfile error\n");
	  exit(-1);
	}
	fprintf(fpcollision,"#1:t_tmp\t2:t_sys\t3:t_ene\t4:global_n_p\t5:global_n\t6:i_sys\t7:n_col\t8:step\t9:E_tot_0\t10:abs_L_0\t11:dE_correct\n");
	fprintf(fpcollision,"%.30e\t%.30e\t%.15e\t%6d\t%6d\t%6d\t%d\t%8e\t%.15e\t%.15e\t%.15e\n",
		t_tmp,
		t_sys,
		t_ene,
		global_n_p,
	...
	fprintf(fpcollision,"\n\n");
	fprintf(fpcollision,"#1:name\t2:x[AU]\t3:y[AU]\t4:z[AU]\t5:|r|[AU]\t6:vx[AU(2pi/yr)]\t7:vy[AU(2pi/yr)]\t8:vz[AU(2pi/yr)]\t9:|v|[AU(2pi/yr)]\t10:ax[AU(2pi/yr)^2]\t11:ay[AU(2pi/yr)^2]\t12:az[AU(2pi/yr)^2]\t13:adotx[AU(2pi/yr)^3]\t14:adoty[AU(2pi/yr)^3]\t15:adotz[AU(2pi/yr)^3]\t16:t_i[yr/2pi]\t17:dt_i[yr/2pi]\t18:mass[Msun]\n");

	for(i=1;i<=global_n;++i){

	  fprintf(fpcollision,"%s\t%.15e\t%.15e\t%.15e\t%.15e\t%.15e\t%.15e\t%.15e\t%.15e\t%.15e\t%.15e\t%.15e\t%.15e\t%.15e\t%.15e\t%.30e\t%.30e\t%.15e\n",
		  ele[i].name,
		  x_0[i][1],
		  x_0[i][2],
		  x_0[i][3],
