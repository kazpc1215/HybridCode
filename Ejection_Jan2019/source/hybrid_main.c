  ...
  if(stat(endfile,&st)==0){  //読み込むファイルの存在確認.
    ...
    */
    ...
    if(stat(collisionfile,&st)==0){  //読み込むファイルの存在確認.
      ...
      fptempread = fopen(collisionfile,"r");
      if(fptempread==NULL){
	fprintf(fplog,"collision temp file error\n");
	return -1;
      }
	...
	if(fgets(buf,sizeof(buf),fptempread)!=NULL){
      ...
      fprintf(fplog,"file: '%s' does not exist.\n",collisionfile);
      return 0;
    ...
    if(stat(collisionfile,&st)==0){  //読み込むファイルの存在確認.
      ...
      fptempread = fopen(collisionfile,"r");
      if(fptempread==NULL){
	fprintf(fplog,"collision file error\n");
	return -1;
      }
